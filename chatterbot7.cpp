...
void CBot::find_match() 
{
	...
	for(int i = 0; i < nKnowledgeBaseSize; ++i) 
	{
		...
		if(index_vector.size() > 0)
		{
			...
			copy( KnowledgeBase[index_vector[0]].response, response_list );
		}
	}
}
...
int main()
{
	std::cout << "Chatterbot v7.0 Copyright (C) 2009 Gonzales Cenelia\n" << std::endl;

	try 
	{
		CBot bot("Chatterbot7");
